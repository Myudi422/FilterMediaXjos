    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ BACK", callback_data=f"next_{req}_{key}_{off_set}"),
             ...
             InlineKeyboardButton(f"📃 {round(int(offset) / 8) + 1} / {round(total / 8)}",
                                  callback_data="pages")]
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"🗓 {round(int(offset) / 8) + 1} / {round(total / 8)}", callback_data="pages"),
             InlineKeyboardButton(text=f"Tutup", callback_data="close_data"),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"🗓 1/{round(int(total_results) / 8)}", callback_data="pages"),
