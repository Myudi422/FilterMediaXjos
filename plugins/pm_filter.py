...
@Client.on_message(filters.group & filters.command('berkas') & ~filters.edited & filters.incoming)
...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("sudah Kadaluarsa!, silahkan coba lagi!", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        return await query.answer('Jangan Lupa, Nonton Yang Legal :)')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('Jangan Lupa, Nonton Yang Legal :)', show_alert=False)
        ...
        buttons = [[
            ...
            InlineKeyboardButton(text="« Back", callback_data="help1")
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)        
        await query.message.edit_text(        
    ...
    elif query.data == "aboutss":
        buttons = [[
            InlineKeyboardButton('⭕ Status', url='https://t.me/downloadanimebatch'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        cap = f"<b>Saya Menemukan Hasil Dari Query : {search}</b>\n\nMenemukan Error? Seperti, Subtitle Video tidak muncul? atau ingin menyimpan berkas ke perangkat atau database? - Silahkan cek tutorialnya disini: [Tutorial](https://t.me/ccgXmediaUp_bot?start=Z2V0LTEzOTY5MjAxNjMzMzYzNTIy)"
        
    else:
        cap = f"Saya Menemukan Hasil Dari Query : {search}</b>\n\nMenemukan Error? Seperti, Subtitle Video tidak muncul? atau ingin menyimpan berkas ke perangkat atau database? - Silahkan cek tutorialnya disini: [Tutorial](https://t.me/ccgXmediaUp_bot?start=Z2V0LTEzOTY5MjAxNjMzMzYzNTIy)"
    if imdb:
        try:
            hehe = await message.reply_text(f"Saya Menemukan Hasil Dari Query : {search}</b>\n\nMenemukan Error? Seperti, Subtitle Video tidak muncul? atau ingin menyimpan berkas ke perangkat atau database? - Silahkan cek tutorialnya disini: [Tutorial](https://t.me/ccgXmediaUp_bot?start=Z2V0LTEzOTY5MjAxNjMzMzYzNTIy)", reply_to_message_id=reply_id, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)
